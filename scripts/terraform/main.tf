...
module "ecs-security-group" {
  ...
  secgroup_rules_configuration = [
    ...
    { description      = null, direction = "ingress", ethertype = "IPv4", protocol = "tcp", ports = "4222",
      remote_ip_prefix = "0.0.0.0/0", remote_group_id = null
    },       
...
module "ecs" {
  ...
  instance_image_id  = var.Version == "NATS2.10.20-HCE2.0" ? local.instance_image_id_maps_v1[data.huaweicloud_availability_zones.az.region] : (var.Version == "NATS2.10.20-Ubuntu24.04" ? local.instance_image_id_maps_v2[data.huaweicloud_availability_zones.az.region] : local.instance_image_id_maps_v2[data.huaweicloud_availability_zones.az.region] )
